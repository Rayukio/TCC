...
const create = async ({ userId, technicianId, serviceId, scheduledAt, address, notes, paymentMethod }) => {
  if (!isValidFutureDate(scheduledAt)) throw { status: 400, message: 'Data de agendamento inválida ou no passado.' };
  ...
  const appointment = await Appointment.create({
    userId, technicianId, serviceId, scheduledAt, address, notes, paymentMethod,
    totalPrice: service.basePrice || technician.pricePerHour,
  });
