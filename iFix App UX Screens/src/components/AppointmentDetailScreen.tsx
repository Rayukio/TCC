      ...
      <div className="bg-[rgb(var(--color-primary))] px-6 pt-12 pb-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="px-4 py-2 rounded-full" style={{ backgroundColor: `${statusColor}15`, color: statusColor }}>
              {statusLabel}
            </div>
          </div>
          <div className="space-y-3 pt-4 border-t border-[rgb(var(--color-border))]">
            <div className="flex items-center gap-3 text-[rgb(var(--color-text-secondary))]">
              <Calendar className="w-5 h-5 text-[rgb(var(--color-primary))]" /><span>{activeOrder.date}</span>
            </div>
            <div className="flex items-center gap-3 text-[rgb(var(--color-text-secondary))]">
              <Clock className="w-5 h-5 text-[rgb(var(--color-primary))]" /><span>{activeOrder.time}</span>
            </div>
            <div className="flex items-center gap-3 text-[rgb(var(--color-text-secondary))]">
              <MapPin className="w-5 h-5 text-[rgb(var(--color-primary))]" /><span>{activeOrder.location}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          ...
          <div className="flex items-center gap-4 mb-4">
            <img src={activeOrder.technician.avatar} alt={activeOrder.technician.name} className="w-16 h-16 rounded-full bg-[rgb(var(--color-primary-light))]" />
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="flex items-center justify-center gap-2 bg-[rgb(var(--color-primary))] text-white px-4 py-3 rounded-xl hover:bg-[rgb(var(--color-primary-dark))] transition-colors">
              <Phone className="w-5 h-5" /><span>Ligar</span>
            </button>
            <button onClick={onChat} className="flex items-center justify-center gap-2 bg-[rgb(var(--color-primary))] text-white px-4 py-3 rounded-xl hover:bg-[rgb(var(--color-primary-dark))] transition-colors">
              <MessageCircle className="w-5 h-5" /><span>Chat</span>
            </button>
          </div>
        </div>

        <button onClick={onTrack} className="w-full bg-gradient-to-r from-[rgb(var(--color-primary))] to-[rgb(var(--color-primary-dark))] text-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow mb-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between text-[rgb(var(--color-text-secondary))]">
              <span>Serviço</span><span>{activeOrder.price}</span>
            </div>
            <div className="flex justify-between text-[rgb(var(--color-text-secondary))]">
              <span>Taxa de serviço</span><span>R$ 0</span>
            </div>
        ...
        <button onClick={handleCancelOrder} className="w-full mt-6 text-red-600 px-6 py-3 rounded-xl border-2 border-red-600 hover:bg-red-50 transition-colors">
