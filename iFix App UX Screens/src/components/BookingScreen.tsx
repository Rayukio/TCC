          ...
          <div className="space-y-2">
            {services.map((service) => (
              <button key={service.id} onClick={() => setSelectedService(service.id)}
                className={`w-full p-4 rounded-xl text-left transition-all ${selectedService === service.id ? "bg-[rgb(var(--color-primary))] text-white shadow-md" : "bg-white text-[rgb(var(--color-text-primary))] shadow-sm"}`}>
                <div className="flex items-center justify-between">
                  <span>{service.name}</span>
                  <span className={selectedService === service.id ? "text-white" : "text-[rgb(var(--color-primary))]"}>{service.price}</span>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {availableDates.map((date, index) => (
              <button key={index} onClick={() => date.available && setSelectedDate(date.date)} disabled={!date.available}
                className={`flex-shrink-0 px-4 py-3 rounded-xl text-center transition-all ${selectedDate === date.date ? "bg-[rgb(var(--color-primary))] text-white" : date.available ? "bg-white text-[rgb(var(--color-text-secondary))] shadow-sm" : "bg-[rgb(var(--color-border))] text-[rgb(var(--color-text-muted))] cursor-not-allowed"}`}>
            ...
            <div className="grid grid-cols-4 gap-2">
              {availableTimes.map((time, index) => (
                <button key={index} onClick={() => setSelectedTime(time)}
                  className={`px-3 py-2 rounded-lg text-center transition-all ${selectedTime === time ? "bg-[rgb(var(--color-primary))] text-white" : "bg-white text-[rgb(var(--color-text-secondary))] shadow-sm"}`}>
                  {time}
                </button>
              ))}
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div className="pt-4 border-t border-[rgb(var(--color-border))] space-y-2">
              <div className="flex justify-between text-[rgb(var(--color-text-secondary))]">
                <span>Serviço:</span><span>{services.find(s => s.id === selectedService)?.name}</span>
              </div>
              <div className="flex justify-between text-[rgb(var(--color-text-secondary))]">
                <span>Data:</span><span>{selectedDate}</span>
              </div>
              <div className="flex justify-between text-[rgb(var(--color-text-secondary))]">
                <span>Horário:</span><span>{selectedTime}</span>
              </div>
              <div className="flex justify-between pt-2 border-t border-[rgb(var(--color-border))]">
                ...
                <span className="text-[rgb(var(--color-primary))]">{services.find(s => s.id === selectedService)?.price}</span>
              </div>
            </div>
          </div>
        )}
